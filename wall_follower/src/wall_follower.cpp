...
bool robot_move(const ROBOT_MOVEMENT move_type)
{
    if (move_type == STOP) {
        ...
        motor_command.angular.z = 0.0;
        motor_command.linear.x = 0.5 * FACTOR;
    }

    else if (move_type == BACKWARD) {
	    ROS_INFO("[ROBOT] I'm going back! \n");
	    motor_command.linear.x = -0.75 * FACTOR;
        motor_command.angular.z = 0.0;
    }
        ...
        motor_command.linear.x = 0.0;
        motor_command.angular.z = 1.0 * FACTOR;
    }
        ...
        motor_command.linear.x = 0.25 * FACTOR;
        motor_command.angular.z = -0.25 * FACTOR;
    }
